...
from cocotb.triggers import First, Event
...
class Producer:
    ...
    async def write(self):
        ...
        for _ in range(self.count):
            ...
            cocotb.log.info(
                f"[WR] : {self.__class__.__name__.upper()} : "
                f"sent     new data : {data:02d}"
            )
...
class Consumer:
    ...
    async def read(self):
        ...
        while True:
            ...
            cocotb.log.info(
                f"[RD] : {self.__class__.__name__.upper()} : "
                f"received new data : {temp:02d}"
            )
            cocotb.log.info("-----------------------------------")
            self.event.set()
...
async def test(dut):
    ...
    writer = cocotb.start_soon(producer.write())
    reader = cocotb.start_soon(consumer.read())
